...
from cars.models import Car, Categories, Fuel, GearBox, Seats
from cars.models import Cities, PersonalDetails
...
class CarForm(forms.ModelForm):
    ...
    fuel = ModelChoiceField(
        ...
        empty_label="Select Fuel Type", required=False,
    ...
    city = ModelChoiceField(
        ...
        empty_label="Select City", required=False,
    ...
    class Meta:
        ...
        widgets = {
            ...
            'year': forms.NumberInput(attrs={
                'min': 1886, 'placeholder': 'Year of Manufacture'}),
            ...
            'large_bags': forms.NumberInput(attrs={
                'placeholder': 'Number of Large Bags'}),
            'small_bags': forms.NumberInput(attrs={
                'placeholder': 'Number of Small Bags'}),
            'air_condition': forms.CheckboxInput(attrs={
    ...
    def clean_year(self):
        ...
        if year < 1886 or year > current_year:
            raise forms.ValidationError(
                f"Enter a valid year between 1886 and {current_year}.")
        return year
...
class PersonalDetailsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'town': forms.TextInput(attrs={
                'placeholder': 'Town/City', 'class': 'form-control'}),
            'county': forms.Select(attrs={
                'placeholder': 'County', 'class': 'form-control'}),
            'eir_code': forms.TextInput(attrs={
                'placeholder': 'Eir Code', 'class': 'form-control'}),
    ...
    def clean_mobile(self):
        ...
        if len(mobile) < 10 or len(mobile) > 15:
            raise forms.ValidationError(
                "Mobile number must be between 10 to 15 digits.")
        return mobile
