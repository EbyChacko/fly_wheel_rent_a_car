    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                try:
                    customer_details = PersonalDetails.objects.get(
                        user=request.user)
                    has_data = any([
                ...
                if not has_data:
                    ...
                    customer_details.title = get_object_or_404(
                        Title, pk=title_id)
                    ...
                    customer_details.date_of_birth = request.POST.get(
                        'date_of_birth')
                    ...
                    customer_details.county = get_object_or_404(
                        County, pk=county_id) if county_id else None
                ...
                booking.drop_off_county = request.session.get(
                    'drop_off_county')
                ...
                booking.booster_seat = request.session.get(
                    'booster_quantity', 0)
                booking.child_seat = request.session.get(
                    'childseat_quantity', 0)
                booking.infant_car_capsule = request.session.get(
                    'infant_quantity', 0)
                booking.booster_total = request.session.get('booster_total', 0)
                booking.childseat_total = request.session.get(
                    'childseat_total', 0)
                ...
                return redirect(
                    'checkout_success', booking_number=booking.booking_number)
        else:
        ...
        if request.user.is_authenticated:
            try:
                customer_details = PersonalDetails.objects.get(
                    user=request.user)
                form.initial = {
                    'title': (customer_details.title.id
                              if customer_details.title else None),
                    ...
                    'county': (customer_details.county.id if
                               customer_details.county else None),
...
def checkout_success(request, booking_number):
    ...
    car_id = request.session.get('car_id')
    car = get_object_or_404(Car, pk=car_id)
    total_extra = (booking.childseat_total + booking.infant_total +
