...
from .models import Cities, Categories, Fuel, GearBox
...
class CarFilterForm(forms.Form):
    ...
    drop_off_date = forms.DateField(label='Drop-off Date', widget=forms.DateInput(attrs={'type': 'date'}))
    drop_off_time = forms.TimeField(label='Drop-off Time', widget=forms.TimeInput(attrs={'type': 'time'}))
    category = forms.ModelChoiceField(queryset=Categories.objects.all(), label='Category', empty_label="Select Category")
    fuel = forms.ModelChoiceField(queryset=Fuel.objects.all(), label='Fuel', empty_label="Select Fuel type")
    gear_box = forms.ModelChoiceField(queryset=GearBox.objects.all(), label='Gear Box', empty_label="Select Gear Box type")
